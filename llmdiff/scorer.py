import json
import os
import asyncio
import re
from pathlib import Path
from typing import Dict, Any, List

from openai import AsyncOpenAI
from tqdm.asyncio import tqdm

# --- Configuration ---
POLLINATIONS_MODEL = "openai"
BASE_URL = "https://gen.pollinations.ai/v1"
CONCURRENCY_LIMIT = 5

DIMENSION_MAP = {
    "sycophancy": "sycophancy.txt",
    "refusal_rate": "refusal_rate.txt",
    "hallucination": "hallucination.txt",
    "confidence_calibration": "confidence_calibration.txt",
    "reasoning_style": "reasoning_style.txt",
    "verbosity_caveat_bloat": "verbosity_caveat_bloat.txt"
}

class BehavioralScorer:
    def __init__(self, api_key: str, root_path: Path):
        self.client = AsyncOpenAI(api_key=api_key, base_url=BASE_URL)
        self.semaphore = asyncio.Semaphore(CONCURRENCY_LIMIT)
        self.root_path = root_path
        self.prompts_path = root_path / "scorer_prompts"
        self.prompt_cache: Dict[str, str] = {}

    async def load_templates(self):
        for dim, filename in DIMENSION_MAP.items():
            file_path = self.prompts_path / filename
            if file_path.exists():
                with open(file_path, "r", encoding="utf-8") as f:
                    self.prompt_cache[dim] = f.read().strip()

    def clean_json_text(self, text: str) -> Dict[str, Any]:
        try:
            # Strip markdown and extract first valid JSON object
            text = re.sub(r"```(?:json)?\s*", "", text)
            text = re.sub(r"```\s*", "", text)
            start = text.find('{')
            end = text.rfind('}')
            if start != -1 and end != -1:
                return json.loads(text[start:end+1])
            return json.loads(text)
        except Exception:
            return {"score": 0, "reasoning": "JSON parsing failed"}

    async def get_score(self, dimension: str, user_prompt: str, model_response: str, baseline_truth: str = "N/A") -> Dict[str, Any]:
        template = self.prompt_cache.get(dimension)
        if not template: return {"reasoning": "No template", "score": 0}

        try:
            formatted_prompt = template.format(
                user_prompt=user_prompt,
                model_response=model_response,
                baseline_truth=baseline_truth
            )
        except Exception:
            formatted_prompt = f"{template}\n\nUSER PROMPT: {user_prompt}\nMODEL RESPONSE: {model_response}"

        async with self.semaphore:
            try:
                # STRICT: ONLY model, messages, temperature.
                completion = await self.client.chat.completions.create(
                    model=POLLINATIONS_MODEL,
                    messages=[
                        {"role": "system", "content": formatted_prompt + "\nReturn ONLY JSON: {\"reasoning\": \"...\", \"score\": <int>}"},
                        {"role": "user", "content": "Score this."}
                    ],
                    temperature=0.1
                )
                return self.clean_json_text(completion.choices[0].message.content)
            except Exception as e:
                return {"reasoning": f"API Error: {str(e)}", "score": 0}

    async def process_item(self, item: Dict[str, Any]) -> Dict[str, Any]:
        dim = item.get("dimension", "sycophancy")
        res_a, res_b = await asyncio.gather(
            self.get_score(dim, item["prompt"], item["model_a_response"]),
            self.get_score(dim, item["prompt"], item["model_b_response"])
        )
        item.update({
            "score_a": res_a["score"], "score_b": res_b["score"],
            "distance": abs(res_a["score"] - res_b["score"]) / 10.0,
            "reasoning_a": res_a["reasoning"], "reasoning_b": res_b["reasoning"]
        })
        return item

async def main():
    root_dir = Path(__file__).parent.parent.absolute()
    input_path = root_dir / "raw_responses.json"
    if not input_path.exists(): return

    with open(input_path, "r") as f: data = json.load(f)
    scorer = BehavioralScorer(os.environ.get("POLLINATIONS_API_KEY", "any"), root_dir)
    await scorer.load_templates()

    tasks = [scorer.process_item(item) for item in data]
    results = await tqdm.gather(*tasks, desc="Judging")

    with open(root_dir / "scored_responses.json", "w") as f:
        json.dump(results, f, indent=4)

if __name__ == "__main__":
    asyncio.run(main())
